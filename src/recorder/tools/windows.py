...
def register_window_tools(mcp):
    ...
    async def window_tools() -> str:
        ...
        output = "Window Management Tools Status\n"
        output += f"{'=' * 40}\n\n"
        ...
        if deps["missing"]:
            ...
            if deps["platform"] == "linux":
                output += "\nTo install:\n"
                output += "  Ubuntu/Debian: sudo apt install wmctrl xdotool\n"
                output += "  Fedora/RHEL:   sudo dnf install wmctrl xdotool\n"
                output += "  Arch:          sudo pacman -S wmctrl xdotool\n"
    ...
    if not is_container_environment():
        ...
        @mcp.tool(
            description="Maximize browser window for demo recording. RECOMMENDED over fullscreen. HOST MODE ONLY. REQUIRES window_title parameter."
        )
