...
def register_video_tools(mcp, backend):
    ...
    @mcp.tool(
        description="Get metadata for a video or audio file: duration, resolution, codecs, file size, and URL."
    )
    ...
    @mcp.tool(
        description="List all media files (videos and audio) in the recordings directory with URLs."
    )
    async def list_media_files() -> str:
        ...
        for i, file_path in enumerate(files, 1):
            ...
            if url:
                output += f"   URL: {url}\n"
            output += (
                f"   Size: {size_mb:.1f} MB | Duration: {duration:.1f}s | Type: {file_type}\n\n"
            )
    ...
    async def adjust_video_to_audio(
        ...
        """
        from ..utils.ffmpeg import get_audio_duration, get_ffmpeg_path
