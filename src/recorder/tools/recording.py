...
def register_recording_tools(mcp, backend):
    ...
    @mcp.tool(
        description="Start video recording. REQUIRES window_title parameter. Perform actions AFTER calling this. Keep scenes short (10-30s)."
    )
    ...
    if hasattr(backend, "list_screens"):
        ...
        @mcp.tool(
            description="List available screens/monitors for recording. Useful for multi-monitor setups on host machine."
        )
