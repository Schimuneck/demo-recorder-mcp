...
class RecordingBackend(ABC):
    ...
    def get_capture_args(
        self, window_title: str, fps: int = 30
    ...
    async def start_recording(
        self, output_path: Path, window_title: str, fps: int = 30
    ) -> RecordingResult:
        ...
        if self._state.is_recording():
            return RecordingResult(
                success=False, message="Recording already in progress. Stop it first."
            )
        ...
        if process.poll() is not None:
            ...
            self._state.reset()
            return RecordingResult(success=False, message=f"Recording failed to start: {error_msg}")

        url = self.get_media_url(output_path)
        return RecordingResult(
            success=True, message="Recording started", file_path=output_path, url=url
        )
    ...
    async def stop_recording(self) -> RecordingResult:
        ...
        if not self._state.is_recording():
            return RecordingResult(success=False, message="No recording in progress.")
    ...
    def get_recording_status(self) -> RecordingResult:
        ...
        if not self._state.is_recording():
            return RecordingResult(success=True, message="No recording in progress.")
