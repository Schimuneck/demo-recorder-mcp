...
class HostBackend(RecordingBackend):
    ...
    def _get_macos_capture_args(
        ...
        screen_index, crop_x, crop_y, scale_factor = self._find_screen_for_window(
            bounds, screen_info
        )
        ...
        return (
            [
                ...
                "uyvy422",
                "-i",
                f"{avfoundation_index}:none",
            ],
            f"crop={width}:{height}:{crop_x_final}:{crop_y_final}",
            None,
        )
...
"""
        try:
            ...
            return (
                screens
                if screens
                else [{"index": 1, "x": 0, "y": 0, "width": 1920, "height": 1080, "scale": 2}]
            )
        except Exception:
    ...
    def _get_linux_capture_args(
        ...
        from ..utils.window_manager import get_window_bounds, get_window_id
